...
`AUDITLINE_DRESS_REHEARSAL` defaults to `true`. Going live requires
...
- Is capped at `AUDITLINE_MAX_CALL_SECONDS` (default 180s).
- Never argues with a denial — if the person says they didn't authorize
  something, the call ends. AuditLine does not try to talk anyone
...
`ChronoAuditor` looks up phone numbers from a separately maintained
...
- `auditline/calle_client.py`'s fixture bank (`default_mock_responses`)
...
A production deployment should add a per-person call budget (e.g. no
more than N verification calls to the same authorizer per day) so a
noisy CI pipeline can't turn into repeated unwanted calls to the same
colleague. This is flagged as a required addition before real
deployment, not built into the hackathon submission — see README
"Extension points."
...
claim. See `auditline/verifier.py` for the exact decision logic.
...
human with normal repository permissions. AuditLine has no merge
